...
Page({
  ...
  onLoad: function(options) { // type 1 超市； 2 水；  3餐饮 ; 0 购物车
    ...
    if (type == 1 || type == 0){
      ...
      shopId = wx.getStorageSync('dining_buy').shop_id;
    }else{
        ...
        success: res => {
          ...
          if (status == 1) {
            ...
            if (type == 1) {
              ...
              var total = parseFloat(o_price) + parseFloat(basisPrice);
  ...
  submitOrders(e) {
    ...
    if (type == 0) {
      ...
      if (len > 0) {
        for (var i = 0; i < len; i++) {
          var num = water[i].num;
          if (water[i].chooses){
            var deliver = water[i].chooses.toString();
          }else{
            var deliver = '';
          }
          ...
          var goods_item = {
            ...
            deliverIds: deliver,
            goodsList: [{
              goodsId: water[i].goodsId,
              goodsNum: 1,
              goodsAttrId: water[i].attributeId,
              goodsAttrName: water[i].info,
            }]
          };
          for (var j = 0; j < num; j++) {
      ...
      if (len2 > 0) {
        var goods_item2 = {
          shopId: 1,
          deliverIds: this.data.deliverBaseId,
          goodsList: []
        };
        for (var i = 0; i < len2; i++) {
          goods_item2.goodsList.push({
            ...
            goodsAttrId: shop_goods[i].attributeId,
            goodsAttrName: shop_goods[i].info,
          })
        }
    ...
    if (type == 1) {
      ...
      var goods = [];
      var goods_item = { shopId: 1, goodsList: '', deliverIds: deliverBaseId };
      var list = [{
        goodsId: shop_goods[0].goods_id,
        goodsNum: shop_goods[0].num,
        attributeId: shop_goods[0].attributeId
      }]
    ...
    if (type == 2) {
      ...
      var len = water.length;
      for (var i = 0; i < len; i++) {
        var deliverIds = water[i].chooses.toString();
        var num = water[i].num;
        var goods_item = {
          shopId: water[i].shopId,
  ...
  d_payend() {
    ...
    if (dining_buy && dining_shop_cart) {
      var len = dining_shop_cart.length;
      var len2 = dining.length;
      for (var i = 0; i < len; i++) {
        if (dining_shop_cart[i].shop_id == dining_buy.shop_id) {
          dining_shop_cart.splice(i, 1);
          len = i;
        }
      }
      for (var j = 0; j < len2; j++) {
        if (dining[j].shop_id == dining_buy.shop_id) {
          dining.splice(j, 1);
          len2 = j;
        }
      }
